...
```php
// Defines a GET route that accepts a user ID in the URL
get('/users/:id', function (Request $req) {
    // Retrieves the user ID from the URL parameters
    $id = $req->param('id');
    
    // Searches for the user in the database by the ID
    $user = User::find($id);
    
    // If the user is not found, returns a response with a 404 status and an error message
    if (!$user) {
    ...
    // If the user is found, loads the profile page with the user data
...
get('/hello/:name', function ($req) {
    response()->send('Hello, ' . $req->param('name'));
});
```
...
get('/protected/:token', $EnsureTokenIsValid, function ($request) {
    $response()->send('Protected content accessed successfully!');
});
```
