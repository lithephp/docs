---
title: Lithe\App  
description: The `Lithe\App` class is the main entry point for applications in Lithe. It manages the application lifecycle, middleware configurations, routes, and responds to HTTP requests.
---

## Description

The `Lithe\App` class is the foundation for creating and managing an application in Lithe. It provides methods for configuring middleware, defining routes, handling HTTP exceptions, and starting the application. Through the `App`, you can manage application configuration, control the execution flow, and provide a response to incoming HTTP requests.

Here is a basic example of how to use an instance of `Lithe\App`:
...
- **Parameters:**
  ...
  - **`$handler`** - Handler to handle the HTTP exception.
...
### `listen(): void`
- **Description:** Allows the application to listen and respond to HTTP requests.
...
- **Parameters:**
  - **`$path`** - Route prefix.
- **Return:** An anonymous object for defining routes with the provided prefix.
...
- **Description:** Registers an event handler for a specific event.
- **Parameters:**
  - **`$event`** - Event name.
  - **`$handler`** - Handler function to be called when the event is emitted.

### `off(string $event, callable $handler): void`
- **Description:** Removes an event handler for a specific event.
- **Parameters:**
  - **`$event`** - Event name.
  - **`$handler`** - Handler function to be removed.

### `emit(string $event, ...$args): void`
- **Description:** Emits an event, calling all registered handlers for that event.
- **Parameters:**
  - **`$event`** - Event name.
  - **`...$args`** - Any additional arguments to be passed to the handlers.
