...
You can use the `make:migration` Line command to generate a database migration. The new migration will be placed in the `src/database/migrations` directory of your project. Each migration file name contains a timestamp that allows Lithe to determine the order of migrations.
