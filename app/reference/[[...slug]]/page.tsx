...
import { FLATTEND_REFERENCE_ROUTES } from "@/lib/routes-config";
...
export async function generateMetadata({
  ...
  const res = await cachedGetMarkdownForSlug(pathName, 'reference');
  if (!res) return null;
...
export function generateStaticParams() {
  return FLATTEND_REFERENCE_ROUTES/*.filter((item) => item.disabled != true)*/.map(
    (item) => ({
  ...
  return (
    <div className="flex items-start gap-12 ">
      <div className="flex-[3] py-10 ">
        <AppBreadcrumb paths={slug} base="Reference" />
      ...
      <Toc path={pathName} category="reference" />
    </div>
  );
}
